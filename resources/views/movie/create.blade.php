<x-app-layout>
    ...
    <form id="createForm" method="POST" action="{{ url('/save')}}" class="md:w-3/4 w-full mx-auto mt-4"> 
        <input id="movieData" type="hidden" value="{{ $movie }}">
        {{ csrf_field() }}
        <input id="userId" type="hidden" value="{{ Auth::id() }}">
        <div class="relative md:w-1/2 w-3/4 mx-auto">
            <input type="search" v-model="searchText" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border-gray-100 border border-gray-300 focus:ring-blue-500 focus:border-blue-500" placeholder="映画タイトルを入力">
            <button type="button" @click="search" class="absolute top-0 right-0 p-2.5 text-sm font-medium text-white bg-blue-700 rounded-r-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300"><svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></button>
        </div>
        <!-- <div v-show="selectedFlag" class="flex justify-center my-2">
            <button type="button" @click="removeMovie" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">映画を外す</button>
        </div> -->
        <span class="text-center block my-5" v-show="searchFlag && searchResultList.length == 0">
            検索がヒットしませんでした。
        </span>
        <div v-show="searchResultList.length > 0" class="md:w-1/2 w-full mx-auto bg-white m-3 p-3 rounded-md">
        ...
        <div v-show="selectedFlag" class="w-full mx-auto mt-4 flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row">
            <img v-if="selectedMovie.poster_path" class="object-contain w-full rounded-t-lg h-96 md:h-full md:w-auto md:rounded-none md:rounded-l-lg" :src="selectedMovie.poster_path">
            <img v-else class="object-contain w-full rounded-t-lg h-96 md:h-full md:w-auto md:rounded-none md:rounded-l-lg" src="{{ asset('img/no_img.png') }}" alt="">
            <div class="flex flex-col justify-between p-4 leading-normal">
                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">@{{ selectedMovie.title }}</h5>
                <p class="mb-3 font-normal text-gray-700">@{{ selectedMovie.overview }}</p>
                
                <div class="flex w-full justify-between mt-2">
                    <div class="w-1/3">
                        <label for="" class="block text-sm font-medium text-gray-900">公開日</label>
                        <p class="mb-3 font-normal text-gray-700">@{{selectedMovie.release_date}}</p>
                    </div>
                ...
                <div class="flex w-full justify-between mt-2" v-show="!existFlag">
                    <div class="w-1/3">
                        ...
                        <input type="date" name="viewing_date" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                    </div>
                    <div class="w-1/3">
                ...
                <div class="mb-3" v-show="!existFlag">
                    ...
                    <textarea name="impression" rows="4" required class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"></textarea>
                </div>
                <div class="mt-2 flex justify-end" v-show="!existFlag">
                    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">登録</button>
                </div>
