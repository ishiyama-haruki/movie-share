<x-app-layout>
    ...
    <div class="w-3/4 mx-auto p-5 mt-5 bg-white">
        @if (Auth::id() == $movieHistory->user_id || $movieHistory->accessible)
            ...
            <div class=" mt-14 border border-gray-200 rounded-lg shadow max-h-176 overflow-y-auto">
                        ...
                        <tbody class="overflow-y-auto">
                                @foreach($movieHistory->comments()->get() as $comment)
                                    <tr class="border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="w-3/5 p-2">{{ $comment->message }}</td>
                                        <td>{{$comment->created_at->format('Y/m/d H:i') }}</td>
                                        <td class="w-1/12 p-2">
                                            @if ($comment->user->id == Auth::id())
                                                <button type="submit" formaction="{{ route('commentDelete', ['id' => $comment->id ]) }}" class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">削除</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <form action="{{ route('commentStore') }}" method="POST" class="flex items-center justify-between m-2">
                    ...
                    <input type="text" name="message" required class="w-2/3 mr-5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="コメントを投稿する">
