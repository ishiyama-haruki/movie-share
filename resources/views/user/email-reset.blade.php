<x-app-layout>
    ...
    <div class="md:w-1/2 w-full mx-auto mt-5 p-5 bg-white border border-gray-200 shadow ">
        <form action="{{ route('emailUpdate', ['id' => Auth::id()]) }}" method="POST">
            {{ csrf_field() }}
            <div class="mb-6">
                <label for="email" class="block mb-2 text-sm font-medium text-gray-900">新しいメールアドレス</label>
                <input type="email" name="email" required value="{{ Auth::user()->email }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="name@flowbite.com" required>
            </div>
